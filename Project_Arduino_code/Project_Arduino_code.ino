...
// Create an instance of the HardwareSerial class for the BARCODE
...
void updateWifiPage()
{
  ...
  wifi_page += "<html lang='en'>";
  wifi_page += "<head>";
  wifi_page += "<meta charset='UTF-8'>";
  wifi_page += "<meta name='viewport' content='width=device-width, initial-scale=1.0'>";
  wifi_page += "<title>Barcode and Date Information</title>";
  wifi_page += "<style>";
  wifi_page += "body { font-family: Arial, sans-serif; margin: 20px; }";
  wifi_page += "h1 { color: #4CAF50; }";
  wifi_page += "p { font-size: 16px; }";
  wifi_page += "</style>";
  wifi_page += "</head>";
  wifi_page += "<body>";

  // Display the barcode information
  wifi_page += "<p><b>Golden Barcode: </b>" + golden_barcode + "</p>";
  wifi_page += "<p><b>Target Barcode: </b>" + target_barcode + "</p>";

  // Compare the barcodes
  if (golden_barcode == target_barcode) 
  {
    wifi_page += "<p><b>Comparison: </b>Identical</p>";
  } 
  else
  {
    wifi_page += "<p><b>Comparison: </b>Different</p>";
  }
  ...
  // Display RTC information on the wifi page
  wifi_page += "<p><b>Date: </b>" + String(dateBuffer) + "</p>";
  wifi_page += "<p><b>Time: </b>" + String(timeBuffer) + "</p>";
  wifi_page += "</body>";
  wifi_page += "</html>";
}
...
void setup() 
{
  ...
  server.on("/", HTTP_GET, [](AsyncWebServerRequest *request){request->send(200, "text/html", wifi_page);}); // Serve a simple web wifi page
...
void loop() 
{
  if (digitalRead(ESP_SWITCH) == LOW)
  {
    ...
    while(1)
    {
      ...
      // Check if there a switch press to scan new golden
      if (target_barcode == "")
      {
        return;
      }
